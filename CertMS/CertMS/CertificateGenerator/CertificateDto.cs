...
namespace CertMS.CertificateGenerator
{
	public class CertificateDto
	{
		...
		{
			return FormatedSubject + Environment.NewLine +
			       FormatedIssuer + Environment.NewLine +
			       string.Join(Environment.NewLine, ExtraProperties.Select(pair => string.IsNullOrWhiteSpace(pair.Value) ? string.Empty : $"{pair.Key}: {pair.Value}")).Trim() + Environment.NewLine +
